...
public class LicencaBean implements Serializable{
    ...
    public void init(){
        ...
        if (isInstalade()){
            if (empresa == null){
                Empresa empresaSearch = new Empresa();
                empresa = empresaRepository.getByAtivo(true);
            }
            if (account == null){
                Account accountSearch = new Account();
                accountSearch.setLogin("admin");               
                List<Account> countAccount = accountRepository.find(accountSearch);
                if (countAccount.size()>0 && countAccount.size() == 1){
                   account = accountRepository.findUnique(account);
                }
                if (countAccount.size()>0 && countAccount.size() > 1){
                   account = accountRepository.find(account).get(0);
                }
            }
        }else{
    ...
    public void save(ActionEvent actionEvent) {  
        ...
        int roleSearch = roleRepository.findCount();
        if (roleSearch <= 0){
            roleRepository.merge(role);
        }
        Role role2 = new Role();
        role2.setRoleName("ROLE_ADMIN");
        int roleSearch2 = roleRepository.findCount();
        if (roleSearch2 <= 0){
            roleRepository.merge(role);
        }
    ...
    public Boolean isInstalade(){
        List<Empresa> countempresa = empresaRepository.find();
        List<Account> countaccount = accountRepository.find();
        System.out.println("Empresa tem: "+countempresa.size()+" itens");
        System.out.println("Account tem: "+countaccount.size()+" itens");
        if (countempresa.size() > 0 && countaccount.size() > 0){   
            instalado = true;            
            return true;
        }else{            
            instalado = false;
