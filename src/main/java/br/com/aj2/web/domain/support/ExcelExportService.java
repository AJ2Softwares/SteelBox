...
public class ExcelExportService {
    ...
    public void export(String template, Map<String, Object> model, String filename) {
        ...
        externalContext.setResponseContentType("application/vns.ms-excel");
    ...
    public Workbook export(String templateClasspathLocation, Map<String, Object> model) {
        ...
        s.start();
        try {
            InputStream resource = getResource(templateClasspathLocation);
    ...
    private InputStream getResource(String templateClasspathLocation) {
        try {
            byte[] templateAsStream;
            if (resources.containsKey(templateClasspathLocation)) {
                templateAsStream = resources.get(templateClasspathLocation);
            } else {
                templateAsStream = IOUtils.toByteArray(new ClassPathResource(templateClasspathLocation).getInputStream());
                resources.put(templateClasspathLocation, templateAsStream);
            }
            return new ByteArrayInputStream(templateAsStream);
        } catch (IOException e) {
